...
namespace FileManipulator.Unused
{
    public class Fileinfo
    {
        ...
        //Constructor
        public Fileinfo(string sourcepath, string destinationbase, bool verbose = false)
        {
            ...
            if (File.Exists(this.sourcepath))
            {
                ...
                createdDate = File.GetCreationTime(this.sourcepath);
                modifieddate = File.GetLastWriteTime(this.sourcepath);
                ...
                md5filehash = BitConverter.ToString(md5.ComputeHash(stream)).Replace("-", string.Empty);
                md5.Clear();
                stream.Close();
                // This is where we release the allocated resources

                if (rawextensions.Contains(filextension))
                {
                    israw = true;
                }
                else
            ...
            {
                createddestpath = Path.Combine(destinationbase, createdDate.ToString("yyyy"), createdDate.ToString("MM-yyyy"), createdDate.ToString("MM-dd-yyyy"), filename);
                modifieddestpath = Path.Combine(destinationbase, modifieddate.ToString("yyyy"), modifieddate.ToString("MM-yyyy"), modifieddate.ToString("MM-dd-yyyy"), filename);
                if (israw)
                {
                    rawcreatetargetpath = Path.Combine(destinationbase, createdDate.ToString("yyyy"), createdDate.ToString("MM-yyyy"), createdDate.ToString("MM-dd-yyyy"), "RAW", filename);
                    rawmodtargetpath = Path.Combine(destinationbase, modifieddate.ToString("yyyy"), modifieddate.ToString("MM-yyyy"), modifieddate.ToString("MM-dd-yyyy"), "RAW", filename);
                }
                else
                {
                    rawcreatetargetpath = "Not Raw";
                    rawmodtargetpath = "Not Raw";
                }
            }
            else
            {
        ...
        private void Writeresults()
        {
            ...
            Console.WriteLine("The file extension is: " + filextension);
            Console.WriteLine("The file path is: " + sourcepath);
            Console.WriteLine("The file was created on: " + createdDate);
            Console.WriteLine("The file was last written on: " + modifieddate);
            Console.WriteLine("The MD5 Hash for this file is: " + md5filehash);
            Console.WriteLine("The file is raw: " + israw);
            Console.WriteLine("The output path for the creation date is: " + createddestpath);
            Console.WriteLine("The output path for the modified date is: " + modifieddestpath);
            Console.WriteLine("The output path for the RAW file creation date is: " + rawcreatetargetpath);
            Console.WriteLine("The output path for the RAW file modified date is: " + rawmodtargetpath + "\n");
            Console.ResetColor();
        }
    }
}